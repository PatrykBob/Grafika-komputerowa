...
enum
{
	...
	TEXTURE_BLONDE,                    // tekstura BLONDE
	TEXTURE_BLONDE_UNC,                // teksura BLONDE nieskompresowana
	TEXTURE_BLONDE_GRAY,               // tekstura BLONDE_GRAY
	TEXTURE_BLONDE_GRAY_UNC,           // tekstura BLONDE_GRAY nieskompresowana
...
GLuint BLONDE, BLONDE_UNC, BLONDE_GRAY, BLONDE_GRAY_UNC;
...
void GenerateTextures()
{
	...
	// wczytanie tekstury BLONDE.tga
	GLboolean error = load_targa("blonde_girl_white_skin_5.tga", width, height, format, type, pixels);

	// b³¹d odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_girl_white_skin_5.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &BLONDE);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, BLONDE);

	// definiowanie tekstury z kompresj¹
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &BLONDE_UNC);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, BLONDE_UNC);
	...
	// wczytanie tekstury BLONDE_GRAY.tga
	error = load_targa("blonde_girl_white_skin_5_gray.tga", width, height, format, type, pixels);

	// b³¹d odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_girl_white_skin_5_gray.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &BLONDE_GRAY);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, BLONDE_GRAY);

	// definiowanie tekstury z kompresj¹
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_LUMINANCE, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &BLONDE_GRAY_UNC);

	// dowi¹zanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, BLONDE_GRAY_UNC);
...
void Menu(int value)
{
	...
	// teksura BLONDE nieskompresowana
	case TEXTURE_BLONDE_UNC:
	{
		texture = BLONDE_UNC;
		DisplayScene();
	}
	...
	// tekstura BLONDE_GRAY
	case TEXTURE_BLONDE_GRAY:
	{
		texture = BLONDE_GRAY;
		DisplayScene();
	}
	...
	// tekstura BLONDE_GRAY nieskompresowana
	case TEXTURE_BLONDE_GRAY_UNC:
	{
		texture = BLONDE_GRAY_UNC;
		DisplayScene();
	}
	break;
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("BLONDE.tga (kompresja)", TEXTURE_BLONDE);
	glutAddMenuEntry("BLONDE.tga (bez kompresji)", TEXTURE_BLONDE_UNC);
	glutAddMenuEntry("BLONDE_GRAY.tga (kompresja)", TEXTURE_BLONDE_GRAY);
	glutAddMenuEntry("BLONDE_GRAY.tga (bez kompresji)", TEXTURE_BLONDE_GRAY_UNC);
